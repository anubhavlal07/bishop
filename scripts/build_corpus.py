...
def build() -> list[dict[str, Any]]:
    ...
    alerts.append(
        alert(
            "TP-16-token-replay",
            rule_name="Session token used by an unrecognised client",
            source="okta",
            severity="high",
            category="identity",
            label="true_positive",
            ...
            why=(
                "A browser session in Manchester continued eleven minutes later from "
                "a different address as python-requests, with no new MFA factor. Both "
                "logins geolocate to the same city, so impossible travel has nothing "
                "to say — the client is the signal. A browser does not become an "
                "HTTP library, and a fresh login would have produced a fresh factor."
            ),
            detected_at=at(hours=6, minutes=41),
            principal={"username": "v.ramanathan", "domain": "CORP"},
            auth_events=[
                {
                    "timestamp": at(hours=6, minutes=30),
                ...
                {
                    "timestamp": at(hours=6, minutes=41),
                    "username": "v.ramanathan",
                    "outcome": "success",
                    "source_ip": "198.51.100.240",
                    "geo": geo("manchester"),
                    "user_agent": "python-requests/2.31.0",
                    "application": "Okta API",
                },
            ],
        )
    )
